...
from unimodal_dataset import MammographyDataset, get_mammo_transforms # <-- Use Mammo specific classes
from utils import EarlyStopping

def setup_logging(config):
    """Sets up logging to file and console for a single run."""
    current_time = datetime.now().strftime('%Y%m%d-%H%M%S')
    log_dir = os.path.join("logs", f"{config['run_name']}_mammo_{current_time}")
...
def get_data_loaders(config):
    """Creates training and validation data loaders for the mammography dataset."""
    image_size = config['training']['image_size']
    train_transforms, val_transforms = get_mammo_transforms(image_size)
    
    # Use MammographyDataset which reads from a CSV
    train_dataset = MammographyDataset(
        csv_file=config['mammo_csv_path'],
    ...
    val_dataset = MammographyDataset(
        csv_file=config['mammo_csv_path'],
        root_dir=config['mammo_data_path'],
        split='validation', # Assuming 'validation' is a split in your CSV
    ...
    # --- NEW: Oversampling Logic for Imbalanced Dataset ---
    logging.info("Setting up WeightedRandomSampler to handle class imbalance...")
    ...
    logging.info(f"Class counts: {class_counts}. Sample weights configured.")
    
    # The training loader will use the sampler. `shuffle` must be False when using a sampler.
...
def train_model(config):
    ...
    current_time = datetime.now().strftime('%Y%m%d-%H%M%S')
    tb_log_dir = os.path.join(config['output']['tensorboard_log_dir'], f"{config['run_name']}_mammo_{current_time}")
    ...
    # We no longer need weighted loss as the sampler handles the imbalance.
    ...
    for epoch in range(config['training']['epochs']):
        ...
        model.eval()
        # (Validation loop logic remains the same)
        running_val_loss, val_labels, val_preds, val_probs = 0.0, [], [], []
        logging.info(f"Validating after epoch {epoch+1}...")
    ...
    cm_save_path = os.path.join(os.path.dirname(tb_log_dir), "final_mammo_confusion_matrix.png")
    cm_fig = plot_confusion_matrix(cm, class_names, cm_save_path)
    writer.add_figure("Final Mammogram Confusion Matrix", cm_fig, global_step=config['training']['epochs'])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train a Unimodal Mammography Classifier.")
